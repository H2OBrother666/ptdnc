...
def _erase_and_write(memory, address, reset_weights, values):
  ...
  add_matrix = torch.matmul(address, values)
  memory += add_matrix

  return memory


class MemoryAccess(torch.nn.modules.RNNCellBase):
  ...
  *   `addressing.FreenessAllocator` for keeping track of memory usage, where
  ...
  def _read_inputs(self, inputs):
    ...
    def _linear(first_dim, second_dim, name, activation=None):
      ...
      linear = torch.nn.Linear(inputs.shape[1:],first_dim * second_dim, name=name)(inputs)
      if activation is not None:
        linear = activation(linear)
      return torch.reshape(linear, [-1, first_dim, second_dim])
    ...
    free_gate = torch.sigmoid(
        torch.nn.Linear(inputs.shape[1:],self._num_reads, name='free_gate')(inputs))

    # g_t^{a, i} - Interpolation between writing to unallocated memory and
    # content-based lookup, for each write head `i`. Note: `a` is simply used to
    # identify this gate with allocation vs writing (as defined below).
    allocation_gate = torch.sigmoid(
        torch.nn.Linear(inputs.shape[1:],self._num_writes, name='allocation_gate')(inputs))

    # g_t^{w, i} - Overall gating of write amount for each write head.
    write_gate = torch.sigmoid(
        torch.nn.Linear(inputs.shape[1:],self._num_writes, name='write_gate')(inputs))
    ...
    read_mode = torch.softmax(_linear(self._num_reads, num_read_modes, name='read_mode'))
    ...
    write_strengths = torch.nn.Linear(inputs.shape[1:],self._num_writes, name='write_strengths')(
        inputs)

    read_keys = _linear(self._num_reads, self._word_size, 'read_keys')
    read_strengths = torch.nn.Linear(self._num_reads, name='read_strengths')(inputs)
  ...
  def _read_weights(self, inputs, memory, prev_read_weights, link):
    ...
    read_weights = (
        content_mode.unsqueeze(2) * content_weights + torch.sum(
            forward_mode.unsqueeze(3) * forward_weights, axis = 2) +
        torch.sum(backward_mode.unsqueeze(3) * backward_weights, axis = 2))
  ...
  def state_size(self):
    ...
    return AccessState(
        memory=torch.tensor([self._memory_size, self._word_size]),
        read_weights=torch.tensor([self._num_reads, self._memory_size]),
        write_weights=torch.tensor([self._num_writes, self._memory_size]),
  ...
  def output_size(self):
    ...
    return torch.tensor([self._num_reads, self._word_size])
