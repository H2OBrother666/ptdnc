...
def _vector_norms(m):
  squared_norms = torch.sum(m * m, 2,True)
...
def weighted_softmax(activations, strengths, strengths_op):
  ...
  """
  transformed_strengths = torch.unsqueeze(strengths_op(strengths), -1)
...
class CosineWeights(torch.nn.Module):
  ...
  def __init__(self,
               num_heads,
               word_size,
               strength_op=torch.nn.functional.softplus ,
               name='cosine_weights'):
  ...
  def forward(self, memory, keys, strengths):
    ...
    dot = torch.matmul(keys, memory.transpose(1,2))
    ...
    norm = torch.matmul(key_norms , memory_norms.transpose(1,2))
...
class TemporalLinkage(torch.nn.Module):
  ...
  def directional_read_weights(self, link, prev_read_weights, forward):
    ...
    expanded_read_weights = torch.stack([prev_read_weights] * self._num_writes,1)
    if forward:
      result = torch.matmul(expanded_read_weights, link.transpose(2,3))
  ...
  def _link(self, prev_link, prev_precedence_weights, write_weights):
    ...
    new_link = write_weights_i.double() * prev_precedence_weights_j.double()
    link = prev_link_scale.double() * prev_link.double() + new_link.double()
    # Return the link with the diagonal set to zero, to remove self-looping
    # edges.
    diag_indices = np.diag_indices(prev_link.shape[3])
    for i in range(batch_size):
      for j in range(num_writes):
        link[i][j][diag_indices] = 0
    return link
  def _precedence_weights(self, prev_precedence_weights, write_weights):
    ...
    write_sum = torch.sum(write_weights, 2, True)
    return (1 - write_sum).double() * prev_precedence_weights.double() + write_weights.double()
  ...
  def state_size(self):
    ...
    return TemporalLinkageState(
        ...
        precedence_weights=torch.tensor([self._num_writes,
                                           self._memory_size]),)
  ...
  *   Decreased by reading, where usage is decreased after reading from a
  ...
  def write_allocation_weights(self, usage, write_gates, num_writes):
    ...
    for i in range(num_writes):
      ...
      usage += ((1 - usage).double() * write_gates[:, i, :].double() * allocation_weights[i].double())

    # Pack the allocation weights for the write heads into one tensor.
    return torch.stack(allocation_weights, 1)
  ...
  def _allocation(self, usage):
    ...
    print('nonusage')
    print(nonusage)
    sorted_nonusage , indices  = torch.sort(nonusage,1,True)
    sorted_nonusage= sorted_nonusage[:,:self._memory_size]
    print('sorted_nonusage')
    print(sorted_nonusage)
    indices = indices[:,:self._memory_size]
    print('indices')
    print(indices)
    ...
    prod_sorted_usage = torch.cumprod(sorted_usage, 1)
    print('prod_sorted_usage')
    print(prod_sorted_usage)
    exclusive_prod_sorted_usage = torch.ones(prod_sorted_usage.shape)
    exclusive_prod_sorted_usage[:,1:] = prod_sorted_usage[:,:-1]
    print('exclusive_prod_sorted_usage')
    print(exclusive_prod_sorted_usage)
    
    sorted_allocation = sorted_nonusage * exclusive_prod_sorted_usage
    ...
    result = util.batch_gather(sorted_allocation, inverse_indices)
    print('result')
    print(result)
    return result
  ...
  def state_size(self):
    ...
    return torch.tensor([self._memory_size])
